...
def plot_heatmap(tensor, title="Heatmap", custom_colors=None, colorbar=True, vmin=0, vmax=0, layer=0,
    ...
    plt.savefig(f"/home/azzhang/streaming-llm/CKA/llama3_layer_{layer}_V.png")
    plt.show()
...
for layer in target_layer:
    print("layer: ", layer)
    cka_matrix = torch.zeros(num_head, num_head)
    mode = "Value"
    # mode = "Key"
    if mode == "Key":
        ...
        num_heads, seq_len, head_dim = K[layer].shape
        ...
        # target_layer = 31
        num_heads, seq_len, head_dim = V[layer].shape
        for i in range(num_heads):
            for j in range(num_heads):
                vi = V[layer][i]
                vj = V[layer][j]
