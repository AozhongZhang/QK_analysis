...
for layer_idx in range(num_layers):
    ...
    w_v = w_v_weight.view(num_heads, -1, w_k_weight.shape[1])
    print(w_k.shape)
    W_k.append(w_k)
    W_v.append(w_v)
...
#     v = collected_v_outputs[i].view(bsz, seq_len, num_head, (hidden//num_head)).transpose(1, 2)
#     k = collected_k_outputs[i].view(bsz, seq_len, num_head, (hidden//num_head)).transpose(1, 2)
#     K.append(k.squeeze(0))
#     V.append(v.squeeze(0))
# print(K[0].shape)

# H_k_0 = K[:,0]
# H_k_0 = [k[7] for k in K]
# H_k_0 = [k[0] for k in V]
# H_k_1 = [k[1] for k in K]
# for i in range(len(collected_k_outputs)):
# print(len(H_k_0))
# print(H_k_0[0].shape)
...
def plot_heatmap(tensor, title="Heatmap", custom_colors=None, colorbar=True, vmin=0, vmax=0, layer=0,
    ...
    plt.savefig(f"/home/azzhang/streaming-llm/CKA_weight/llama2_layer_{layer}_W_v.png")
    plt.show()

# target_layer = [0, 1, 5, 10, 20, 31]
# select_list = [K[0],K[1],K[5],K[10],K[20],K[31]]
# global_min = min(matrix.min().item() for matrix in select_list)
# global_max = max(matrix.max().item() for matrix in select_list)
# del select_list
CKA_matrix = []
...
for layer in range(32):
    ...
    if mode == "Key":
        ...
        num_heads, seq_len, head_dim = W_k[layer].shape
        ...
        # target_layer = 31
        num_heads, seq_len, head_dim = W_v[layer].shape
        for i in range(num_heads):
            for j in range(num_heads):
                vi = W_v[layer][i]
                vj = W_v[layer][j]
    ...
    CKA_matrix.append(cka_matrix)

print(len(CKA_matrix))
print(CKA_matrix[0].shape)
global_min = min(matrix.min().item() for matrix in CKA_matrix)
global_max = max(matrix.max().item() for matrix in CKA_matrix)

for i in range(len(CKA_matrix)):
    if torch.equal(CKA_matrix[0], CKA_matrix[1]):
        print("+++++")
    ...
    plot_heatmap(CKA_matrix[i], title=f"CKA Matrix ({mode}-Cache)", colorbar=True, 
                 vmin=global_min, vmax=global_max, layer=i, 
                 x_label="head", y_label="head", 
