...
def register_input_hessian_to_model(model, tokenizer, args, dev):
    """
    计算每层 Linear 的输入协方差矩阵 X^T X,并直接注册到 module.hessian_matrix 属性中。
    无返回值，修改模型内部状态。
    """
    # import os
    # import torch
    ...
    # os.makedirs("cache/hessian", exist_ok=True)
    # cache_file = f"cache/hessian/{model_id.replace('/', '_')}_hessian_inplace.pt"

    # if os.path.exists(cache_file) and args.use_cache:
    #     logger.info(f"[Hessian] Load in-place Hessians from cache: {cache_file}")
    #     layerwise_data = torch.load(cache_file, map_location="cpu")

    #     # Load into model modules
    #     for i, layer_data in enumerate(layerwise_data):
    #         layer = model.model.layers[i]
    #         subset = find_layers(layer)
    #         for name in subset:
    #             if name in layer_data:
    #                 subset[name].hessian_matrix = layer_data[name]
    #     return

    logger.info("[Hessian] Begin computing and registering per-module Hessian (X^T X)...")
    ...
    if "llama" in model_id or "mistral" in model_id or "vicuna" in model_id or "longchat" in model_id:
        ...
        raise NotImplementedError("Unknown model family.")
    ...
    dtype = next(iter(model.parameters())).dtype
    inps = torch.zeros(
        (len(calib_loader), 2048, model.config.hidden_size),
        dtype=dtype,
        device=dev,
    )
    cache = {'i': 0, 'attention_mask': None, 'position_ids': None}
    ...
    for i in tqdm(range(len(layers))):
        ...
        for name in subset:
            if "k_proj" in name or "v_proj" in name:
                subset[name].hessian_matrix = torch.zeros(
                    subset[name].in_features,
                    subset[name].in_features,
                    device=dev,
                    dtype=torch.float32,
                )
        ...
        def hook(module, input, output):
            with torch.no_grad():
                ...
                module.hessian_matrix += inp.T @ inp
                del inp, output
                torch.cuda.empty_cache()
        ...
        for name in subset:
            if "k_proj" in name or "v_proj" in name:
                h = subset[name].hessian_matrix.cpu()
                subset[name].hessian_matrix = h
                del h
        
        torch.cuda.empty_cache()
        # 保存本层所有需要的模块的 hessian（用于缓存）
        # layer_data = {}
        # for name in subset:
        #     if "k_proj" in name or "v_proj" in name:
        #         layer_data[name] = subset[name].hessian_matrix.cpu()

        # layerwise_data.append(layer_data)
        inps = outs
        layers[i] = layer.cpu()
        del subset
        del layer
    ...
    # torch.save(layerwise_data, cache_file)
    # logger.info(f"[Hessian] In-place Hessians saved to: {cache_file}")


def compress_model_exact_hessian(model, tokenizer, args, dev, selection_result):
    """
    Replace Linear to HeadwiseLowRankModule（使用模块内部注册的 hessian_matrix）。
    
    Args:
        model: 原始模型
        selection_result: dict {layer_name: [ranks]}，每个线性层的 per-head rank 分配
    """
    logger.info("[ExactCompress] Compressing model with exact Hessian-aware decomposition...")
    register_input_hessian_to_model(model, tokenizer, args, dev)
    ...
    logger.info(f"[ExactCompress] #target layers: {len(selection_result)}")

    for layername, selected_head_rank in tqdm(selection_result.items()):
        if layername not in module_dict:
            logger.warning(f"[ExactCompress] Layer {layername} not found in model, skip.")
            continue

        raw_linear = module_dict[layername]
        info = linear_info[raw_linear]

        # if not hasattr(raw_linear, "hessian_matrix"):
        #     logger.warning(f"[ExactCompress] No hessian_matrix found in {layername}, skip.")
        #     continue

        # hessian = raw_linear.hessian_matrix

        logger.debug(f"[ExactCompress] Decomposing {layername} with rank={selected_head_rank}")
        
        # replace HeadwiseLowRankModule
        head_wise_exact_linear = HeadwiseLowRankModule.from_linear_exact_new(
            old_module=raw_linear,
            ranks=selected_head_rank,
            # hessian=hessian
        )

        
        setattr(info["father"], info["name"], head_wise_exact_linear)

def compress_model_exact_hessian_new(model, tokenizer, args, dev, selection_result):
    """
    Replace Linear to HeadwiseLowRankModule (hessian_matrix)。
    ...
    for layername, selected_head_rank in tqdm(selection_result.items()):
        ...
        # replace HeadwiseLowRankModule
        head_wise_exact_linear = HeadwiseLowRankModule.from_linear_exact(
            old_module=raw_linear,
            ranks=selected_head_rank,
            # hessian=hessian
        )
...
def compress_model_exact_hessian_similiarity(model, tokenizer, args, dev, selection_result, group_to_rows_all_layers):
    """
    Replace Linear to HeadwiseLowRankModule (hessian_matrix)。
    ...
    for layername, selected_head_rank in tqdm(selection_result.items()):
        ...
        raw_linear = module_dict[layername]
        info = linear_info[raw_linear]
        group_to_rows = group_to_rows_all_layers.get(layername, None)
...
def compress_model_whiten_sim(model, tokenizer, args, dev, selection_result, group_to_rows_all_layers):
    logger.info("Compressing model with whiten decomposition...")
    # NOTE(brian1009): Prepare whiten scaling matrix
    get_whiten_scale_matrix(model, tokenizer, args, dev)
    # Compress the model
    ...
    for layername, selected_head_rank in tqdm(selection_result.items()):
        ...
        head_wise_svd_linear = HeadwiseLowRankModule.from_linear_whiten_new(
            old_module=raw_linear,
            ranks=selected_head_rank,
            group_to_rows=group_to_rows,
        )
...
def compress_model_Kwhitensim_Vcalnosim(model, tokenizer, args, dev, selection_result, group_to_rows_all_layers):
    logger.info("Compressing model with whiten decomposition...")
    # NOTE(brian1009): Prepare whiten scaling matrix
    get_whiten_scale_matrix(model, tokenizer, args, dev)
    # Compress the model
    ...
    for layername, selected_head_rank in tqdm(selection_result.items()):
        ...
        if 'k_proj' in layername:
            head_wise_linear = HeadwiseLowRankModule.from_linear_whiten_new(
                old_module=raw_linear,
                ranks=selected_head_rank,
                group_to_rows=group_to_rows,
            )
            ...
            head_wise_linear = HeadwiseLowRankModule.from_linear_exact_new(
            old_module=raw_linear,
            ranks=selected_head_rank,
            group_to_rows=group_to_rows,
        )
        setattr(info["father"], info["name"],  head_wise_linear)


# Wrapper for different decompose methods
def compress_model(model, tokenizer, args, dev, selection_result, group_to_rows_all_layers):
    ...
    elif args.decompose_method == "svd_cal":
        compress_model_exact_hessian(model, tokenizer, args, dev, selection_result)
    elif args.decompose_method == "whiten_sim":
        compress_model_whiten_sim(model, tokenizer, args, dev, selection_result, group_to_rows_all_layers)
    elif args.decompose_method == "svd_cal_sim":
        compress_model_exact_hessian_similiarity(model, tokenizer, args, dev, selection_result, group_to_rows_all_layers)
    elif args.decompose_method == "svd_whiten_cal":
        compress_model_Kwhitensim_Vcalnosim(model, tokenizer, args, dev, selection_result, group_to_rows_all_layers)
    else:
