...
def compress_model_whiten_sim(model, tokenizer, args, dev, selection_result):
    logger.info("Compressing model with whiten decomposition...")
    # NOTE(brian1009): Prepare whiten scaling matrix
    get_whiten_scale_matrix(model, tokenizer, args, dev)
    # Compress the model
    ...
    for layername, selected_head_rank in tqdm(selection_result.items()):
        ...
        if 'k_proj' in layername:
            head_wise_svd_linear = HeadwiseLowRankModule.from_linear_whiten_simreorder(
...
def compress_model_whiten_fisher_aware(model, tokenizer, args, dev, selection_result):
    logger.info("Compressing model with whiten decomposition...")
    # NOTE(brian1009): Prepare whiten scaling matrix
    get_whiten_scale_matrix(model, tokenizer, args, dev)
    # Compress the model
    ...
    for layername, selected_head_rank in tqdm(selection_result.items()):
        ...
        head_wise_svd_linear = HeadwiseLowRankModule.from_linear_whiten_fisher_aware(
            raw_linear,
            selected_head_rank
        )
        # if 'k_proj' in layername:
...
def compress_model(model, tokenizer, args, dev, selection_result):
    ...
    elif args.decompose_method == "svd_whiten_sim_reorder":
        compress_model_whiten_sim(model, tokenizer, args, dev, selection_result)
    elif args.decompose_method == "svd_whiten_fisher_aware":
        compress_model_whiten_fisher_aware(model, tokenizer, args, dev, selection_result)
    else:
