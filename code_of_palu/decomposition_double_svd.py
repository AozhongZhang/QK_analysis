...
from .cluster_double_svd import collect_KV
...
def compress_model_whiten(model, tokenizer, args, dev, selection_result):
    ...
    KV_for_first_svd = collect_KV(model, tokenizer)
    # Compress the model
    ...
    for layername, selected_head_rank in tqdm(selection_result.items()):
        ...
        # head_wise_svd_linear = HeadwiseLowRankModule.from_linear_whiten_double_svd(
        #     raw_linear,
        #     selected_head_rank
        # )
        matrix_first_svd = KV_for_first_svd[layername].squeeze(0)
        # head_wise_svd_linear = HeadwiseLowRankModule.from_linear_whiten_double_svd(
        #     raw_linear,
        #     selected_head_rank,
        #     matrix_first_svd
        # )
        head_wise_svd_linear = HeadwiseLowRankModule.from_linear_whiten_H_projection(
            raw_linear,
            selected_head_rank,
            # matrix_first_svd
        )
