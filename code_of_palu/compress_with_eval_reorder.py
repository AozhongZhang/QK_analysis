...
# from palu.rank_search_palu_difgroup import rank_search
from tqdm import tqdm
# from palu.decomposition import compress_model
from palu.cluster_search_reorder_nouniform import collect_headwise_KV, get_group_to_rows_all_layers_by_cka, compute_similiarity, build_ordered_group_to_rows, collect_W_kv
# from palu.rank_search_new import allocate_rank_from_group_rows_and_fisher, allocate_rank_from_group_rows_and_fisher_V, convert_group_to_rows_to_layername, convert_selection_result_to_layername, merge_kv_from_model_name, merge_kv_select_results
from palu.rank_search_new import allocate_rank_from_group_rows_and_fisher, merge_kv_from_model_name, allocate_rank_from_group_rows_and_fisher_new
from palu.rank_search_svd_energy import allocate_rank_from_group_rows_and_energy, allocate_rank_from_group_rows_and_fisher_new
...
def compress(args):
    ...
    # K_list, V_list = collect_W_kv(model)
    K_dict = build_ordered_group_to_rows(K_list)
    V_dict = build_ordered_group_to_rows(V_list)
    # K_dict = get_group_to_rows_all_layers_by_cka(K_list, compute_similiarity)
    # V_dict = get_group_to_rows_all_layers_by_cka(V_list, compute_similiarity)
    del K_list, V_list
    # print(K_dict)
    Dict = merge_kv_from_model_name(model, K_dict, V_dict)
    print(Dict)
    
    # select_result, final_rank_K, total_rank_K = allocate_rank_from_group_rows_and_energy(model, tokenizer, Dict, 0.5)
    ...
    compress_model(model, tokenizer, args, args.device, search_results, Dict)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default='svd_cal_sim',
        choices=['whiten', 'svd', 'svd_cal', 'whiten_sim','svd_cal_sim', 'svd_whiten_cal'],
